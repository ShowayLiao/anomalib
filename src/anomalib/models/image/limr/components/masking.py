...
def mask_upsample(mask, H_low, H_high, batch_size):
    ...
    up_keep, up_restore = mask2ids(up_mask)
    return up_keep.unsqueeze(0).repeat(batch_size, 1), up_mask, up_restore.unsqueeze(0).repeat(batch_size, 1)


def mask_everylayer(x, mask_ratio):
    H, W = x.shape[2], x.shape[3]
    _, mask, _ = random_masking(x, mask_ratio)
    mask_for_patch5 = 1 - mask.reshape(-1, 7, 7).unsqueeze(1)
    ids_keep_5, ids_restore_5 = mask2ids(mask_for_patch5, path_size=1)
    ids_keep_1, mask_for_patch1, ids_restore_1 = mask_upsample(mask, H // 2, H // 32, x.shape[0])
    ids_keep_2, mask_for_patch2, ids_restore_2 = mask_upsample(mask, H // 4, H // 32, x.shape[0])
    ids_keep_3, mask_for_patch3, ids_restore_3 = mask_upsample(mask, H // 8, H // 32, x.shape[0])
    ids_keep_4, mask_for_patch4, ids_restore_4 = mask_upsample(mask, H // 16, H // 32, x.shape[0])
    ...
    ids_restore_list = [
        ids_restore_1, ids_restore_2, ids_restore_3, ids_restore_4,
